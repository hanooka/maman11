...
def euclidean_distance(board1, board2):
    """ returns distance matrix, where each value,
    is the euclidian distance of tile x in board1, from tile x in board2 """
    size = board1.shape[0]
    ...
    for i in range(size):
        for j in range(size):
            ...
            distance_matrix[i, j] = np.sqrt((pos_board1[0] - pos_board2[0]) ** 2 + (pos_board1[1] - pos_board2[1]) ** 2)
...
def manhattan_distance(board1, board2):
    """ returns distance matrix, where each value,
    is the manhattan distance of tile x in board1, from tile x in board2 """
    size = board1.shape[0]
...
def _manhattan_distance(board1, board2):
    """ A faster implementation of the manhattan distance leveraging numpy superiority """
    size = board1.shape[0]
    flat_board1 = board1.flatten()
    flat_board2 = board2.flatten()
    pos_board1 = np.unravel_index(np.argsort(flat_board1), board1.shape)
    pos_board2 = np.unravel_index(np.argsort(flat_board2), board2.shape)
    distance_matrix = np.abs(pos_board1[0] - pos_board2[0]) + np.abs(pos_board1[1] - pos_board2[1])
...
class TilesGame():
    ...
    def calculate_distance(board1, board2, dist_metric='l2'):
        if str(dist_metric).lower() == 'l2':
            """ fail implementation.. this uses tiles values instead of positions """
            return np.sqrt(np.sum(np.power(board1 - board2, 2)))

        elif str(dist_metric).lower() == 'l1':
            """ lol what is this? this is not admissible :> same logic error of values instead of positions """
            return np.sum(np.abs(board1 - board2))

        elif str(dist_metric).lower() == 'euc':
            dist_mat = euclidean_distance(board1, board2)
